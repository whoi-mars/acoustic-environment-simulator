...
#----------------------------------------------------------------------------
#
# You must first run "make", as if to compile kraken, to make the obj files.
# These all get linked in below.
    OBJDIR="obj"
    OBJ="                      \
	$OBJDIR/zsecx.o		\
	$OBJDIR/zbrentx.o	\
...
#----------------------------------------------------------------------------
            ...
            COPTIMFLAGS='-O3 -DNDEBUG -march=athlon-xp'
            CDEBUGFLAGS='-g'
#           
            CXX='g++'
            CXXFLAGS='-fPIC -ansi -D_GNU_SOURCE -pthread '
            CXXLIBS="$RPATH $MLIBS -lm"
            CXXOPTIMFLAGS='-O3 -DNDEBUG -march=athlon-xp'
            CXXDEBUGFLAGS='-g'
#
#           NOTE: g77 is not thread safe
            ...
            FOPTIMFLAGS='-O3 -march=athlon-xp'
            FDEBUGFLAGS='-g'
#
            LD="$COMPILER"
            LDEXTENSION='.mexglx'
#            LDFLAGS="$OBJ -lg2c -pthread -shared -m32 -Wl,--version-script,$TMW_ROOT/extern/lib/$Arch/$MAPFILE"
            LDFLAGS="$OBJ -pthread -shared -m32 -Wl,--version-script,$TMW_ROOT/extern/lib/$Arch/$MAPFILE -L/usr/local/g95-install/lib/gcc-lib/x86_64-unknown-linux-gnu/4.0.3/ -lf95"
            LDOPTIMFLAGS='-O3 -march=athlon-xp'
            LDDEBUGFLAGS='-g'
#
            POSTLINK_CMDS=':'
#----------------------------------------------------------------------------
...
#----------------------------------------------------------------------------
            RPATH="-Wl,-rpath-link,$TMW_ROOT/bin/$Arch"
            CC='suncc'
            CFLAGS='-xO5 -xarch=amdsse4a -xcache=64/64/2:512/64/16:1536/64/48 -xchip=amdfam10 -fns -ftrap=common -xlibmil -xlibmopt -nofstore -xregs=frameptr -fsimple=2 -Xc -xprefetch_level=3 -fPIC -D_GNU_SOURCE'
#            CC='gcc-4.1'
...
#
            LD="$COMPILER"
            LDEXTENSION='.mexa64'
            LDFLAGS="$OBJ -xO5 -xarch=amdsse4a -xcache=64/64/2:512/64/16:1536/64/48 -xchip=amdfam10 -fns -ftrap=common -xlibmil -xlibmopt -nofstore -xregs=frameptr -fsimple=2 -shared -Wl,--version-script,$TMW_ROOT/extern/lib/$Arch/$MAPFILE -L/opt/sun/sunstudio12.1/lib/amd64/ -lfsu -lfui"
#            LDFLAGS="$OBJ -shared -Wl,--version-script,$TMW_ROOT/extern/lib/$Arch/$MAPFILE -L/usr/local/g95-install/lib/gcc-lib/x86_64-suse-linux-gnu/4.0.3/ -lf95"
            LDDEBUGFLAGS='-g'
#
            POSTLINK_CMDS=':'
#----------------------------------------------------------------------------

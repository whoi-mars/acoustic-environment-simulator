...
#----------------------------------------------------------------------------
#
# You must first run "make", as if to compile eigenray, to make the obj files.
...
#            LDFLAGS="$OBJ -lg2c -pthread -shared -m32 -Wl,--version-script,$TMW_ROOT/extern/lib/$Arch/$MAPFILE"
            LDFLAGS="$OBJ -pthread -shared -m32 -Wl,--version-script,$TMW_ROOT/extern/lib/$Arch/$MAPFILE"
...
#----------------------------------------------------------------------------
            RPATH="-Wl,-rpath-link,$TMW_ROOT/bin/$Arch"
            CC='gcc'
            CFLAGS='-fPIC -fno-omit-frame-pointer -ansi -D_GNU_SOURCE -pthread -fexceptions -fno-leading-underscore'
...
#
            LD="$COMPILER"
            LDEXTENSION='.mexa64'
#            LDFLAGS="$OBJ -pthread -shared -Wl,--version-script,$TMW_ROOT/extern/lib/$Arch/$MAPFILE -L/usr/local/g95-install/lib/gcc-lib/x86_64-suse-linux-gnu/4.0.3/ -lf95"
            LDFLAGS="$OBJ -pthread -shared -Wl,--version-script,$TMW_ROOT/extern/lib/$Arch/$MAPFILE -L/usr/lib64/gcc/x86_64-suse-linux/4.3/  -lgfortran"
            LDOPTIMFLAGS='-O'
            LDDEBUGFLAGS='-g'
#
            POSTLINK_CMDS=':'

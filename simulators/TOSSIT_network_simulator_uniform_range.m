...
% get environmental bounds based on source radius with some buffer
UL = reckon(config.TOSSIT_LATLONS_RAW(1,1),...
            config.TOSSIT_LATLONS_RAW(1,2),...
            km2deg(sqrt(2*(config.R_max^2)) + 2),...
            315);
LL = reckon(config.TOSSIT_LATLONS_RAW(1,1),...
            config.TOSSIT_LATLONS_RAW(1,2),...
            km2deg(sqrt(2*(config.R_max^2)) + 2),...
            225);
LR = reckon(config.TOSSIT_LATLONS_RAW(1,1),...
            config.TOSSIT_LATLONS_RAW(1,2),...
            km2deg(sqrt(2*(config.R_max^2)) + 2),...
            135);
UR = reckon(config.TOSSIT_LATLONS_RAW(1,1),...
            config.TOSSIT_LATLONS_RAW(1,2),...
            km2deg(sqrt(2*(config.R_max^2)) + 2),...
            45);
env_bounds = [UL; LL; LR; UR];
    ...
    for i_c_sed = 1:L_c_sed
        ...
        for i_H = 1:L_H
            ...
            for i_loc = 1:L_locs
                ...
                % sample location
                theta = 2 * pi * rand;
                r =  sqrt(config.R_min^2 + rand * (config.R_max^2 - config.R_min^2)); % config.R * sqrt(rand);
                x_s_km = r * cos(theta);
                y_s_km = r * sin(theta);
                [y_s, x_s] = find_in_meshgrid(mesh_x, mesh_y, x_s_km, y_s_km);
